...
def get_downloads_html(session: HTMLSession, url: str) -> list:
    """
    ...
    items = soup.select_one("div.software-items")
    return items.select("div.software-item") if items is not None else []
...
def parse_html(html: list, region: str) -> list:
    ...
    for item in html:
        ...
        try:
            version = re.search(
                r"([A-Z0-9+]+_[0-9]+(?:.|_)[A-Z]+(?:.|_)[0-9]+)",
                item.select("div.software-field")[0].text,
            ).group(1)
        ...
        try:
            date = (
                item.select("div.software-field")[1].text.strip().split(": ")[1].strip()
            )
        ...
        try:
            md5 = (
                item.select("div.software-field")[3].text.strip().split(": ")[1].strip()
            )
        except IndexError:
            md5 = "Unknown"
...
def merge_yaml(regions: dict):
    ...
    for file in yaml_files:
        with open(f"data/{file}/{file}.yml", "r", encoding="UTF-8") as yaml_file:
            updates = yaml.load(yaml_file, Loader=yaml.FullLoader)
            for update in updates:
                yaml_data.append(update)
    with open("data/latest.yml", "w", encoding="UTF-8") as output:
...
def merge_archive():
    ...
    """
    yaml_files = [
        x for x in sorted(glob("data/archive/*.yml")) if not x.endswith("archive.yml")
    ]
    yaml_data = []
    for file in yaml_files:
        with open(file, "r", encoding="UTF-8") as yaml_file:
            yaml_data.append(yaml.load(yaml_file, Loader=yaml.FullLoader))
    with open("data/archive/archive.yml", "w", encoding="UTF-8") as output:
        yaml.dump(yaml_data, output, allow_unicode=True)
...
def diff_yaml(filename: str) -> list:
    ...
    try:
        with open(
            f"data/{filename}/{filename}.yml", "r", encoding="UTF-8"
        ) as new, open(
            f"data/{filename}/old_{filename}", "r", encoding="UTF-8"
        ) as old_data:
    ...
    if first_run is False:
        ...
        if changes:
            return [
                i for i in latest for codename in changes if codename == i["codename"]
            ]
...
def tg_post(session: HTMLSession, message: str) -> int:
    """
    ...
    telegram_req = session.post(telegram_url, params=params)
...
def archive(update: dict):
    ...
    version = update["version"]
    codename = (
        link.split("/")[-1].split("_")[0]
        if "sign" not in link
        else link.split("/")[-1].split("_")[1]
    )
    try:
        with open(f"data/archive/{codename}.yml", "r", encoding="UTF-8") as yaml_file:
            data = yaml.load(yaml_file, Loader=yaml.FullLoader)
            data[codename].update({version: link})
            data.update({codename: data[codename]})
            with open(f"data/archive/{codename}.yml", "w", encoding="UTF-8") as output:
...
def git_commit_push():
    ...
    system(
        'git add *.yml */*.yml && git -c "user.name=RealmeCI" -c '
        '"user.email=RealmeCI@example.com" '
        'commit -m "sync: {}" && '
        " \
           "
        "git push -q https://{}@github.com/RealmeUpdater/"
        "realme-updates-tracker.git HEAD:master".format(today, GIT_OAUTH_TOKEN)
    )
...
def main():
    ...
    """
    session = HTMLSession()
    with open("data/regions.yml", "r", encoding="UTF-8") as yaml_file:
    ...
    for region_code, region in regions.items():
        ...
        downloads_html = get_downloads_html(session, f"{R_SITE}/{region_code}/{PAGE}")
    ...
    for region in list(regions.values()):
        ...
        if changes:
            for update in changes:
                ...
                # print(message)
                status = tg_post(session, message)
                if status == 200:
